...
public class ClusteringServiceImpl implements ClusteringService {
    ...
    public ClusteringServiceImpl(BloodPressureRepo bloodPressureRepo, PatientRepo patientRepo, BloodPressureService bloodPressureService, TreatmentService treatmentService) {
    ...
    public List<PatientsCluster> clusterPatients() {
            ...
            List<Patient> patientsWithRecords = patients.stream().filter(p -> p.getBloodPressures().size() != 0 && p.getAcceptedSharingData() != null && p.getAcceptedSharingData()).collect(Collectors.toList());
            List<Patient> patientsWithoutRecords = patients.stream().filter(p -> p.getBloodPressures().size() == 0 && p.getAcceptedSharingData() != null && p.getAcceptedSharingData()).collect(Collectors.toList());
            ...
            if (data.length > 1) {
                ...
                for (int i = 0; i < data.length; i++) {
                    for (int j = 0; j < data.length; j++) {
                        if (i != j) {
                            double distance = distance(data[i], data[j], patientsWithRecords.get(i).getCurrentType().toString(), patientsWithRecords.get(j).getCurrentType().toString());
                ...
                for (Map.Entry<Integer, List<Patient>> entry : clustersMap.entrySet()) {
                    ...
                    clusters.add(new PatientsCluster(
                            ...
                            entry.getValue().stream()
                                    .map(p -> {
                                        String medicalCond = "";
                                        if (p.getCurrentType().toString().equalsIgnoreCase("hypotension")) {
                                            medicalCond = "Hipotensiune";
                                        } else if (p.getCurrentType().toString().equalsIgnoreCase("hypertension")) {
                                            medicalCond = "Hipertensiune";
                                        }

                                        List<TreatmentResponseDto> treatments = new ArrayList<>();
                                        if (medicalCond.length() > 1) {
                                            treatments = treatmentService.getPatientOngoingTreatments(p.getEmail(), medicalCond);
                                        }
                                        return treatments;
                                    })
                                    .collect(Collectors.toList())
    ...
    public PatientsCluster getPatientCluster(String email) {
        ...
        PatientsCluster result = allClusters.stream().filter(c -> c.getPatientEmails().contains(email)).findAny().get();
        return result;
    }

    public double distance(double[] s, double[] t, String categoryS, String categoryT) {
        int n = s.length;
        int m = t.length;
